...
#define DEFAULT_BROKER_PORT         1880
...
static int temperature_perc = 50;
		 ...
		 if(state == STATE_NET_OK){
		  ...
		  if(state==STATE_CONNECTED){
			  ...
			  if (status == MQTT_STATUS_OUT_QUEUE_FULL) {
                		LOG_ERR("Tried to subscribe to Temperature but command queue was full!\n");
                		PROCESS_EXIT();
              		  }
		  ...
		  if(state == STATE_SUBSCRIBED){
			...
			//publish temperature percentage every 30 seconds
  			...
  			PROCESS_WAIT_EVENT_UNTIL(etimer_expired(&publish_timer));
		    	...
		    	temperature_perc = (rand() % 20);
				
		    	sprintf(app_buffer, "{\"TEMP\":%d}",temperature_perc);
